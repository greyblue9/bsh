...
public class Loader extends ClassLoader {
    ...
    private void init(ClassPool cp) {
      acl = new AndroidClassLoader(Thread.currentThread().getContextClassLoader());
...
// ;
        if (domain == null) {
          // ;
            return acl.defineClass(name, classfile); //, 0, classfile.length);
        } else {
            return acl.defineClass(name, classfile); //, 0, classfile.length, domain);
        }
    }
