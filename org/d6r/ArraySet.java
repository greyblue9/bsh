...
efficient than a traditional `java.util.HashSet'. The design is very similar to
...
HashSet, since lookups require a binary search and adds and removes require 
inserting and deleting entries in the array. For containers holding up to hundreds 
of items, the performance difference is not significant, less than 50%.

Because this container is intended to better balance memory use, unlike most other
standard Java containers it will shrink its array as items are removed from it. 

Currently, you have no control over this shrinking -- if you set a capacity and then
remove an item, it may reduce the capacity to better match the current size. In the 
future an explicit call to set the capacity should turn off this aggressive
shrinking behavior.
*/
             ...
             Set<E>
{
      ...
      synchronized ((ArraySet.class)) {
        if (sTwiceBaseCache != null) {
          ...
          try {
            ...
            sTwiceBaseCacheSize--;
            if (DEBUG) {
              Log.d(TAG, "Retrieving 2x cache " + mHashes + " now have " + sTwiceBaseCacheSize + " entries");
            }
            return;
          ...
          Log.wtf(TAG, "Found corrupt ArraySet cache: [0]=" + array[0] + " [1]=" + array[1]);
      ...
      synchronized ((ArraySet.class)) {
        if (sBaseCache != null) {
          ...
          try {
            ...
            sBaseCacheSize--;
            if (DEBUG) {
              Log.d(TAG, "Retrieving 1x cache " + mHashes + " now have " + sBaseCacheSize + " entries");
            }
            return;
          } catch (ClassCastException e) {
          }
  ...
  */
  ...
  public <T> T[] toArray(T[] array) {
    if (array.length < mSize) {
      @SuppressWarnings("unchecked") T[] newArray = (T[]) Array.newInstance(array.getClass().getComponentType(), mSize);
      array = newArray;
    }
  ...
  */
  ...
  /**
  Remove all values in the array set that do notexist in the given collection.
