...
import org.d6r.ClassVisitorFactoryImpl;
...
public class Dex2Java {
  ...
  public static class Args {
    ...
    // Output
    @Dex2Java.Out public Map<Method, Exception> exceptionMapOutput;
    @Dex2Java.Out public final List<Throwable> errors 
      = new ArrayList<Throwable>();
    @Dex2Java.Out public BiConsumer<String, byte[]> classBytesHandler;
    @Dex2Java.Out public V3InnerClzGather afa = new V3InnerClzGather();
    @Dex2Java.Out public ClassVisitorFactory classVisitorFactory;
    @Dex2Java.Out public V3 v3;
    @Dex2Java.Out Map<String, byte[]> byteOutputMap;
  }
  ...
  public Dex2Java(DexFileReader reader) {
    super();
    this.reader = reader;
  }
  ...
  public static Dex2Java from(DexFileReader reader) {
    return new Dex2Java(reader);
  }
  
  public static Dex2Java from(byte[] in) throws IOException {
    return from(new DexFileReader(in));
  }

  public static Dex2Java from(DataIn in) throws IOException {
    return from(new DexFileReader(in));
  }

  public static Dex2Java from(File in) throws IOException {
    return from(new DexFileReader(in));
  }

  public static Dex2Java from(InputStream in) throws IOException {
    return from(new DexFileReader(in));
  }

  public static Dex2Java from(String in) throws IOException {
  ...
  public static byte[] convert(byte[] inputBytes) {
    if (inputBytes.length < 10) {
      throw new IllegalArgumentException(String.format(
        "Dex2Java.convert(byte[]): length<10 (%d)", inputBytes.length
      ));
    }
    
    System.err.printf(
      "[INFO] Dex2Java.convert(byte[%d] { %c %c %c ... })\n",
      inputBytes.length,
      inputBytes[0], inputBytes[1],
      inputBytes[2] >= 'A' && inputBytes[2] <= 'z'
        ? Character.valueOf((char) inputBytes[2]).toString()
        : String.format("0x%02x", inputBytes[2])
    );
    return DexConverter.dex2jar(inputBytes).getValue();
  }
  
  
  public static byte[] convertOne(byte[] dexBytes, String className) {
    if (dexBytes.length < 10) {
      throw new IllegalArgumentException(String.format(
        "Dex2Java.convertOne(byte[],String): length<10 (%d)", dexBytes.length
      ));
    }
    
    System.err.printf(
      "[INFO] Dex2Java.convertOne(byte[%s] { %s %s %s ... }, \"%s\")\n",
      Integer.valueOf(dexBytes.length),
      Character.valueOf((char)dexBytes[0]),
      Character.valueOf((char)dexBytes[1]),
      (dexBytes[2] >= 'A' && dexBytes[2] <= 'z')
        ? Character.valueOf((char) dexBytes[2]).toString()
        : String.format("0x%02x", dexBytes[2]),
      className
    );
    return convertOne(className, true, true);
  }
  
  
  public static byte[] convertOne(String className, boolean withDeps,
  boolean addToClasspath)
  {
    try {
      URL classUrl = CollectionUtil.getClassPool().find(className);
      final byte[] classBytes;
      if (classUrl == null) {      
        byte[] jarBytes = ClassInfo.importDexClasses( //;
          className, withDeps, addToClasspath
        );
        classBytes = ZipUtil.toByteArray(
          jarBytes, ClassInfo.classNameToPath(className, "class")
        );
        return classBytes;
      }
      return IOUtils.toByteArray(classUrl);
    } catch (IOException ioEx) {
      throw Reflector.Util.sneakyThrow(ioEx);
    }
  }
  ...
  {
    final Dex2Java.Args args = new Dex2Java.Args();
  ...
  public static Args convert(@Nonnull Args args, Dex dex) {
    ...
    Dex2Java d2j = d2jCache.get(dex);
    if (d2j == null) {
      d2j = Dex2Java.from(reader);
  ...
  static Map<Dex, DexFileReader> readerCache
    = new IdentityHashMap<>();
  static Map<Dex, Dex2Java> d2jCache
    = new IdentityHashMap<>();
    ...
    throws IOException 
  {
    ...
    ClassVisitorFactory cvf 
      = new ClassVisitorFactoryImpl((Dex2Java) this, this.onDone);
  ...
  public Dex2Java withOption(int constOptionBits, boolean enable) {
    this.v3Config = (enable)
      ? this.v3Config |   constOptionBits
      : this.v3Config & (~constOptionBits);
    return this;
  }
  public Dex2Java withFlag(String name, Object value) {
    Reflect.setfldval(this, name, value);
    return this;
  }
  public Dex2Java verbose() {
    return this.withFlag("verbose", true);
  }
  public Dex2Java verbose(boolean flag) {
    return this.withFlag("verbose", flag);
  }
  public Dex2Java reUseReg(boolean b) {
    return this.withOption(V3.REUSE_REGISTER, b);
  }
  public Dex2Java topoLogicalSort(boolean b) {
    return this.withOption(V3.TOPOLOGICAL_SORT, b);
  }
  public Dex2Java optimizeSynchronized(boolean b) {
    return this.withOption(V3.OPTIMIZE_SYNCHRONIZED, b);
  }
  public Dex2Java printIR(boolean b) {
    return this.withOption(V3.PRINT_IR, b);
  }
  public Dex2Java reUseReg() {
    return this.withOption(V3.REUSE_REGISTER, true);
  }
  public Dex2Java optimizeSynchronized() {
    return this.withOption(V3.OPTIMIZE_SYNCHRONIZED, true);
  }
  public Dex2Java printIR() {
    return this.withOption(V3.PRINT_IR, true);
  }
  public Dex2Java withExceptionHandler(DexExceptionHandler exHandler) {
    return this.withFlag("exceptionHandler", exHandler);
  }
  public Dex2Java withDoneHandler(BiConsumer<String, byte[]> onDone) {
    return this.withFlag("onDone", onDone);
  }
  public Dex2Java withArgs(Args args) {
    return this.withFlag("args", args);
  }
  public Dex2Java skipDebug(boolean enable) {
    this.readerConfig = (enable)
      ? this.readerConfig 
      : this.readerConfig & (~DexFileReader.SKIP_DEBUG);
    return this;
  }
  public Dex2Java skipDebug() {
    this.readerConfig &= (~DexFileReader.SKIP_DEBUG);
    this.readerConfig &= (~DexFileReader.SKIP_DEBUG);
    return this;
  }
