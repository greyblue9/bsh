...
public class LocalvariablesNamesEnhancer {
    ...
    public static List<String> lookupParameterNames(Constructor constructor) {
        try {
            ...
            ClassPool classPool = CollectionUtil.getClassPool();
            ...
            throw new RuntimeException("Cannot extract parameter names", e);
        }
    }
    ...
    // @Override
    public byte[] enhanceThisClass(CtClass ct) throws Exception {

        CtClass ctClass = CollectionUtil.getClassPool()
          .get(ct.getName());
        ...
        String className = String.format(
          ...
          ct.getName(),
          Long.toString(
            new Random(System.nanoTime()).nextLong() ^ Long.MAX_VALUE,
            32
        ...
        for (CtMethod method : ctClass.getDeclaredMethods()) {
            ...
            for (int i = 0; i < method.getParameterTypes().length + (Modifier.isStatic(method.getModifiers()) ? 0 : 1); i++) {
                ...
                try {
                    ...
                    new RuntimeException(String.format(
                      ...
                      + "while applying localvariables to %s.%s, "
                      + "param %s", 
                      e, ctClass.getName(), method.getName(), i
                    ), e).printStackTrace();                    
                }
            }
            ...
            for (int i = 0; i < localVariableAttribute.tableLength(); i++) {
                ...
                /* DEBUG
                IO.write(ctClass.toBytecode(), new File("/tmp/lv_"+applicationClass.getName()+".class"));
                ctClass.defrost();
                 */
                ...
                try {
                    ...
                    jv.compileStmnt("org.d6r.LocalvariablesNamesEnhancer$LocalVariablesNamesTracer#addVariable(\"" + aliasedName + "\", " + name + ");");
                    ...
                    // Bon chaque instruction de cette mM-CM-^CM-BM-)thode
                    while (codeIterator.hasNext()) {
                        ...
                        // (en fait la frame commence M-CM-^C  localVariableAttribute.startPc(i)-1 qui est la premiM-CM-^CM-BM-(re affectation
                        //  mais aussi l'initialisation de la variable qui est deja tracM-CM-^CM-BM-) plus haut, donc on commence M-CM-^C  localVariableAttribute.startPc(i))
            ...
            method.insertBefore("org.d6r.LocalvariablesNamesEnhancer$LocalVariablesNamesTracer#enter();");
            method.insertAfter("org.d6r.LocalvariablesNamesEnhancer$LocalVariablesNamesTracer#exit();", true);
    ...
    public static class LocalVariablesNamesTracer {
        ...
        public static void checkEmpty() {
            if (localVariables.get() != null && localVariables.get().size() != 0) {
                    new RuntimeException(String.format(
                      "[ERROR] LocalVariablesNamesTracer.checkEmpty, "
                      + "constraint violated (%s)",
                      localVariables.get().size()
                    )).printStackTrace();
            }
        }
