...
class ZipRecord extends CpioEntry {
  ...
  public char getType() {
    ...
    while (i < Zip.modes.length - 4) { 
      i += 4; 
      int mask = ((Integer) Zip.modes[i]).intValue(); 
      if ((mask & mode) == 0) continue; 
      glyph = (Character) Zip.modes[i+1]; 
      if (glyph != null) { 
        cachedType = glyph;
  ...
  public long getLong(String key) {
    ...
    if (val instanceof byte[]) {
      val = Zip.stringFromBytes((byte[]) val)
...
public class Zip implements List<ZipRecord> {
  
  TarInputStream tar;
  byte[] block = new byte[512];
  
  Map<String, ZipRecord> entries;
  ...
  public Zip(String path, String regex) {
    this(path);
  ...
  public Zip(String path, Pattern ptrn) {
    this(path);
  ...
  */
  ...
  public int read(int maxEnt) {
    if (entries == null) {
      entries = new ListOrderedMap<String, ZipRecord>();
    }
    int entriesRead = 0;
    ...
    (entriesRead < maxEnt && !done) 
    {
      ...
      ZipRecord entry = (this.entry = new ZipRecord()); 
      ...
      try {
        ...
        while (i + 3 < layout.length && !done) { 
          ...
          &&  ! "content".equals(key)) { 
            ...
            if (radix != 0) {
              value = Long.valueOf(
                Zip.stringFromBytes(bytes), radix
              );
            } else {
              switch (bytes.length) {
  ...
  public Map<String, ? extends ZipRecord> getEntries() {
    if (entries == null) {
      read();
    }
    return entries;
  ...
  public ZipRecord getEntry(String name) {
    ZipRecord te = this.getEntries().get(name);
    ...
    for (Map.Entry<String, ZipRecord> e: entries.entrySet()) 
    {
  ...
  public InputStream getInputStream(ZipRecord te) {
  ...
  @Override
  public void add(int location, ZipRecord entry) {
    throw new UnsupportedOperationException();
    //getEntries().put(location, entry.getName(), entry);
  }
  @Override
  public boolean add(ZipRecord entry) {
    boolean contains 
  ...
  @Override
  public boolean addAll(int location, Collection<? extends ZipRecord> toAdd) {
    throw new UnsupportedOperationException();
    /*Collections.addAll(
      getEntries().values(), toAdd.toArray(new ZipRecord[0])
    );*/
  }
  @Override
  public void clear() {
  ...
  public boolean equals(Object object) {
    ...
    if (!(object instanceof Zip)) return false;
    return System.identityHashCode(this)
        == System.identityHashCode(object);
  }
  @Override
  public ZipRecord get(int location) {
    return (ZipRecord) 
  ...
  @Override
  public Iterator<ZipRecord> iterator() {
    return (Iterator<ZipRecord>)
  ...
  @Override
  public ListIterator<ZipRecord> listIterator() {
    throw new UnsupportedOperationException();
    //return getEntries().values().listIterator();
  }
  @Override
  public ListIterator<ZipRecord> listIterator(int location) {
    throw new UnsupportedOperationException();
    //return getEntries().values().listIterator(location);
  }
  @Override
  public ZipRecord remove(int location) {
    ZipRecord entry = get(location);
    remove(location);
    return entry;
  }
  @Override
  public ZipRecord set(int location, ZipRecord object) {
    throw new UnsupportedOperationException();
    //return getEntries().values().set(location, object);
  }
  @Override
  public List<ZipRecord> subList(int start, int end) {
    throw new UnsupportedOperationException();
  ...
  @Override
  public boolean addAll(Collection<? extends ZipRecord> toAdd)
  {
    Collections.addAll(this, toAdd.toArray(new ZipRecord[0]));
    return true;
  }
  @Override
