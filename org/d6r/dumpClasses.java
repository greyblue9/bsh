...
public class dumpClasses {
 ...
 = new HashSet<Type>();
  ...
  public static class DumpClassesOptions 
    extends DumpOptions<dumpClasses, GenericDeclaration> 
  {
  ...
  (Interpreter env, CallStack callstack, Class cls) {
    DumpClassesOptions opts = new DumpClassesOptions();
    env.println(
      dumpClasses(null, cls, new DumpClassesOptions())
    );
  }
  
  public static void invoke
  (Interpreter env, CallStack callstack, Object o) 
  {
    ...
    env.println(dumpClasses(obj, cls, verbose));
  }
  
  public static void invoke(Interpreter env, CallStack callstack, 
  Object o, Class<?> cls) {
    
    env.println(dumpClasses(o, cls, verbose));
  }
  ...
  (Object o, Class<?> cls, boolean verbose) {
    ...
    DumpClassesOptions opts 
 = new DumpClassesOptions();
    opts.verbose = verbose;
    return dumpClasses(o, cls, opts);
  }
  
  public static String dumpClasses
  (Object o, Class<?> cls, DumpClassesOptions options) {
 ...
 = new Member[
    ...
    try { 
      _type = (ParameterizedType) typeMap.get(cls);
    } catch (Throwable e) {}
 ...
 = cls.getGenericSuperclass();
    ...
    for (int icIdx=0; icIdx<innerClasses.length; icIdx++) 
    {
      ...
      if (options.visited.contains(innerCls)) continue; 
      options.visited.add(innerCls); 
      String innerDump = colorize("  // As inner class " + typeToString(innerCls), "1;30");
      try {
        innerDump = dumpClasses(o, innerCls, verbose);
      } catch (Throwable e) { 
        innerDump = dumpClasses(o, innerCls, verbose);
      }
      innerDump = "  " + innerDump.replace("\n", "\n  ");
      sb.append(innerDump);
    }
    ...
    for (Object extended: extendsArr)
    {
      ...
      if (options.visited.contains(iCls)) continue; 
      options.visited.add(iCls); 
      sb.append(lf);
      sb.append( dumpClasses(o, iCls, options) );
    }
    

    
    if (superCls != null && superCls != Object.class
     && ! options.visited.contains(superCls)) {

      options.visited.add(superCls); 
      sb.append((char)0x0A);
      sb.append(
        dumpClasses(o, superCls, options)
      );
    }
    //sb.append(lf);
    return sb.toString();
