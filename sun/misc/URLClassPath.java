...
/**
 * This class is used to maintain a search path of URLs 
 * for loading classes and resources from both JAR files
 *  and directories.
 *
...
public class URLClassPath {
  ...
  static class JarLoader extends Loader {
    ...
    static JarFile checkJar(JarFile jar) 
      throws IOException
    {
      return jar;
    }
    ...
    Resource getResource(final String name, boolean check, Set<String> visited) {
      ...
      while (count < jarFilesList.size()) {
        ...
        while (count < size) {
          ...
          try {
            ...
            String urlNoFragString 
              = URLUtil.urlNoFragString(url);
            if (
              (newLoader 
                = (JarLoader) lmap.get(urlNoFragString)) 
                == null) 
            {
                          
              // no loader has been set up for this jar
              // file before
              newLoader 
                = new JarLoader(url, handler, lmap);
              /* this newly opened jar file has its own
                index, merge it into the parent's index,
                aking into account the relative path.
              */
         ...
         */
          boolean visitedURL 
            = !visited.add(URLUtil.urlNoFragString(url));
          if (!visitedURL) {
            try {
         ...
         /* Verify that at least one other resource with
            the same package name as the lookedup
            resource is present in the new jar
          */ //;
